...
module.exports = class UnbanCommand extends BaseCommand {
  ...
  async run(client, message, args) {
    ...
    if (!message.guild.me.hasPermission("BAN_MEMBERS")) return message.channel.send("I do not have permission to unban members. Please edit my role.");
    ...
    message.guild.fetchBans().then(async bans => {
      if (bans.size === 0) return message.channel.send('There are no bans on this server.');
      let bUser = bans.find(b => b.user.id === userID);
      if(!bUser) return message.channel.send('The user ID stated is not banned.');
      await message.guild.members.unban(bUser.user, reason).catch(err => {
        console.log(err);
        return message.channel.send("Something went wrong while unbanning the member")
      }).then(() => {
        message.channel.send(`I successfully unbanned ${args[0]}`)
      })
    })
